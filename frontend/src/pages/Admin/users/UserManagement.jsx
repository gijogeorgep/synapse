...
const UserManagement = () => {
    ...
    const [editFormData, setEditFormData] = useState({ name: '', email: '', phoneNumber: '', class: '' });
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-7xl mx-auto p-4 md:p-8">
                    ...
                    <div className="bg-white rounded-3xl p-6 w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleUpdateUser} className="space-y-4">
                            <div>
                                ...
                                <input type="text" value={editFormData.name} onChange={(e) => setEditFormData({ ...editFormData, name: e.target.value })} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-cyan-500" required />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Email</label>
                                <input type="email" value={editFormData.email} onChange={(e) => setEditFormData({ ...editFormData, email: e.target.value })} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-cyan-500" required />
                            </div>
                            <div>
                            ...
                            {selectedUser.role === 'student' && (
                                <div>
                                    ...
                                    <select value={editFormData.class} onChange={(e) => setEditFormData({ ...editFormData, class: e.target.value })} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-cyan-500">
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <button type="submit" className="flex-1 py-2.5 bg-cyan-600 text-white rounded-xl font-medium hover:bg-cyan-700 shadow-sm transition-colors">Save Changes</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {isDeleteModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200 text-center">
                        ...
                        <p className="text-slate-500 mt-2">Are you sure you want to delete <span className="font-bold text-slate-700">{selectedUser.name}</span>? This action cannot be undone.</p>
                        <div className="flex gap-3 mt-8">
                            ...
                            <button onClick={confirmDelete} className="flex-1 py-3 bg-rose-600 text-white rounded-xl font-medium hover:bg-rose-700 shadow-lg shadow-rose-200 transition-colors">Delete User</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {isBlockModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-3xl p-6 w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-slate-800">{selectedUser.isBlocked ? 'Unblock User' : 'Block User'}</h2>
                            <button onClick={() => setIsBlockModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X className="w-5 h-5 text-slate-400" /></button>
                        </div>
                        {!selectedUser.isBlocked ? (
                            <div className="space-y-4">
                                <p className="text-sm text-slate-500">Provide a reason for blocking <span className="font-bold">{selectedUser.name}</span>. They won't be able to log in until unblocked.</p>
                                <textarea
                                    value={blockReason}
                                ...
                                />
                                <div className="flex gap-3 pt-4">
                                    <button onClick={() => setIsBlockModalOpen(false)} className="flex-1 py-2.5 border border-slate-200 text-slate-600 rounded-xl font-medium hover:bg-slate-50 transition-colors">Cancel</button>
                                    <button onClick={() => confirmBlockAction(true)} className="flex-1 py-2.5 bg-rose-600 text-white rounded-xl font-medium hover:bg-rose-700 transition-colors">Confirm Block</button>
                                </div>
                            </div>
                        ) : (
                            <div className="space-y-4 text-center">
                                <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl text-left">
                                    <p className="text-xs font-bold text-amber-700 uppercase tracking-wider mb-1">Current Block Reason</p>
                                    <p className="text-slate-700 text-sm italic">"{selectedUser.blockReason || 'No reason provided'}"</p>
                                </div>
                                <p className="text-sm text-slate-500 mt-4">Are you sure you want to unblock <span className="font-bold">{selectedUser.name}</span>?</p>
                                <div className="flex gap-3 pt-4">
                                    <button onClick={() => setIsBlockModalOpen(false)} className="flex-1 py-2.5 border border-slate-200 text-slate-600 rounded-xl font-medium hover:bg-slate-50 transition-colors">Cancel</button>
                                    <button onClick={() => confirmBlockAction(false)} className="flex-1 py-2.5 bg-emerald-600 text-white rounded-xl font-medium hover:bg-emerald-700 transition-colors">Confirm Unblock</button>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-2">Manage Students and Teachers in the platform.</p>
                </div>
                <button
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="px-4 py-2 bg-white text-slate-600 text-sm font-bold rounded-xl border border-slate-200 shadow-sm whitespace-nowrap hidden sm:block">
                                {filteredUsers.length} <span className="font-normal text-slate-500">Found</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="text-center py-16 bg-slate-50 rounded-2xl border border-slate-100 border-dashed">
                        ...
                        <h3 className="text-slate-500 font-medium">No users match your filters</h3>
                        <button
                        ...
                        <table className="w-full text-left border-collapse min-w-[900px]">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-100">
                                    <th className="py-4 text-sm font-semibold text-slate-600 px-6">User Id</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filteredUsers.map((user) => (
                                    <tr key={user._id} className="hover:bg-slate-50/80 transition-colors group">
                                        ...
                                        <td className="py-4 px-6">
                                            <span className={`px-2.5 py-1 text-xs font-semibold rounded-md border ${user.role === 'student'
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="flex items-center justify-end space-x-2">
                                                ...
                                                <button
                                                    ...
                                                    className={`p-2 rounded-xl transition-colors border border-transparent ${user.isBlocked
