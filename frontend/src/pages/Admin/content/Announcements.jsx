    ...
    return (
        <div className="p-6 md:p-10 space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700 max-w-6xl mx-auto font-outfit">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 className="text-4xl font-black text-slate-800 tracking-tightest leading-none">Broadcasts</h1>
                    <p className="text-slate-400 mt-3 text-sm font-medium tracking-wide">Manage and deploy platform-wide notifications.</p>
                </div>
                <button 
            ...
            {status.message && (
                <div className={`p-4 rounded-2xl flex items-center space-x-3 text-xs font-black uppercase tracking-widest animate-in slide-in-from-top-4 duration-300 ${status.type === 'success' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-rose-50 text-rose-600 border border-rose-100'}`}>
                    {status.type === 'success' ? <CheckCircle2 className="w-4 h-4 shadow-sm" /> : <AlertCircle className="w-4 h-4 shadow-sm" />}
            ...
            {loading ? (
                ...
                <div className="text-center py-40 bg-white rounded-[2.5rem] border border-slate-50 shadow-sm">
                    ...
                    <h3 className="text-xl font-black text-slate-300 uppercase tracking-widest mb-1">No Broadcast History</h3>
                    <p className="text-slate-400 font-medium text-sm">Your manual broadcasts will appear here.</p>
                </div>
            ) : (
                <div className="grid gap-4">
                    {announcements.map((ann) => (
                        <div key={ann._id} className="bg-white rounded-[1.5rem] p-6 border border-slate-100/60 hover:border-cyan-200/50 hover:shadow-2xl hover:shadow-cyan-500/5 transition-all group relative">
                             <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleDelete(ann._id)} className="p-2.5 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all border border-transparent hover:border-rose-100">
                                    <Trash2 className="w-4 h-4" />
                                </button>
                             </div>
                             
                             <div className="flex gap-6 items-start">
                                <div className={`p-4 rounded-2xl ${ann.targetRole === 'admin' ? 'bg-slate-50 text-slate-400' : ann.targetRole === 'teacher' ? 'bg-amber-50 text-amber-500' : 'bg-cyan-50 text-cyan-500'}`}>
                                    <Megaphone className="w-5 h-5" />
                                </div>
                                <div className="flex-1 pr-10">
                                    <div className="flex items-center gap-3 mb-2 flex-wrap">
                                        <h3 className="font-black text-slate-800 tracking-tight">{ann.title}</h3>
                                    ...
                                    <p className="text-slate-500 font-medium leading-relaxed whitespace-pre-wrap text-sm mb-4">{ann.content}</p>
                                    <div className="flex items-center gap-4 text-[9px] text-slate-300 font-bold uppercase tracking-tighter">
                                        <div className="flex items-center gap-1.5 bg-slate-50 px-3 py-1.5 rounded-full">
                                            ...
                                            <span>Sent {new Date(ann.createdAt).toLocaleDateString()}</span>
                                        </div>
                                    </div>
                                </div>
                             </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 transition-all duration-500">
                    <div className="bg-white rounded-[2rem] p-10 w-full max-w-xl shadow-3xl animate-in zoom-in-95 duration-500 border border-slate-100">
                        <div className="flex justify-between items-center mb-10">
                            <div>
                                <h2 className="text-2xl font-black text-slate-800 tracking-tightest uppercase">New Broadcast</h2>
                                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-1">Manual Notification Trigger</p>
                            </div>
                            <button onClick={() => setIsModalOpen(false)} className="p-3 bg-slate-50 hover:bg-slate-100 rounded-2xl transition-all border border-slate-100 group"><X className="w-5 h-5 text-slate-400 group-hover:rotate-90 transition-transform" /></button>
                        </div>

                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="space-y-6">
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest block">Broadcast Title</label>
                                    <input 
                                        name="title" 
                                ...
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 mb-3 uppercase tracking-widest block">Choose Audience</label>
                                ...
                                {audienceType !== 'all' && (
                                    <div className="grid grid-cols-2 gap-4 animate-in fade-in duration-500">
                                        <div className={audienceType === 'role' ? 'col-span-2' : ''}>
                                            <label className="text-[9px] font-black text-slate-400 mb-2 uppercase tracking-widest block">Target Role</label>
                                            <select 
                                                name="targetRole" 
                                        ...
                                        {audienceType === 'classroom' && (
                                            <div>
                                                <label className="text-[9px] font-black text-slate-400 mb-2 uppercase tracking-widest block">Target Classroom</label>
                                                <select 
                                ...
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest block">Detailed Content</label>
                                    <textarea 
                                        name="content" 
                            ...
                            <button type="submit" className="w-full py-5 bg-slate-900 text-white font-black uppercase tracking-[0.3em] text-[10px] rounded-2xl hover:bg-cyan-600 transition-all shadow-2xl shadow-cyan-100 active:scale-[0.98]">
                                Initiate Broadcast
                            </button>
                        </form>
                    </div>
