  ...
  try {
    ...
    if (classroom) {
      ...
      if (
        ...
        const studentEmails = classroomData.students
          .filter((s) => s != null)
          .map((s) => s.email);
  ...
  try {
    ...
    if (!classroomId) {
      ...
      if (req.user.role === "student" && req.user.userType === "independent") {
        ...
        const enrolledIds = (req.user.enrolledClassrooms || []).map((id) =>
          id.toString(),
        );
      ...
      // Institutional students: filter by their enrolled classrooms
        ...
        req.user.userType === "institutional"
      ) {
        const studentClassrooms = await Classroom.find({
          students: req.user._id,
        });
...
export const createExamWithQuestions = async (req, res) => {
  ...
  if (!title) {
    return res
      .status(400)
      .json({ message: "Exam title is required even for drafts." });
  }

  if (
    !isDraft &&
    (!subject || !classroom || (isTimed !== false && !duration))
  ) {
    return res
      .status(400)
      .json({
        message:
          "Please provide all required fields: subject, classroom, and duration (when timer enabled).",
      });
  }
  ...
  if (!isDraft) {
    ...
    let qCountForScoring =
      parseInt(totalQuestions) || (questions ? questions.length : 0);

    // If sections exist, use the sum of attendQuestions for scoring
    if (sections && Array.isArray(sections) && sections.length > 0) {
      const sumAttend = sections.reduce(
        (acc, s) => acc + (parseInt(s.attendQuestions) || 0),
        0,
      );
      if (sumAttend > 0) {
    ...
    if (mPerQ <= 0 || tMarks <= 0) {
      return res
        ...
        .json({
          message:
            "Marks per question and Total marks must be greater than zero when publishing.",
        });
    }
    ...
    if (examCategory !== "practice" && !date) {
      return res
        .status(400)
        .json({ message: "Date is required for scheduled exams." });
    }
...
export const addQuestion = async (req, res) => {
  const { questionText, options, correctAnswer, explanation, status } =
    req.body;
...
export const saveDraftQuestion = async (req, res) => {
  ...
  if (!questionText && (!options || options.length === 0)) {
    return res
      .status(400)
      .json({
        message: "Provide at least a question text or options to save a draft.",
      });
  }
...
export const publishQuestion = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Not authorized to publish this question." });
    }
...
export const getExamQuestions = async (req, res) => {
  try {
    ...
    const includeDrafts =
      req.query.includeDrafts === "true" || isAdminOrTeacher;
  ...
  try {
    ...
    if (!isDraft) {
      ...
      if (
        ...
        const sumAttend = currentSections.reduce(
          (acc, s) => acc + (parseInt(s.attendQuestions) || 0),
          0,
        );
        if (sumAttend > 0) {
    ...
    exam.duration =
      duration !== undefined ? parseInt(duration) || 0 : exam.duration;
    exam.isTimed = isTimed !== undefined ? isTimed : exam.isTimed;
    exam.totalMarks =
      totalMarks !== undefined ? parseFloat(totalMarks) || 0 : exam.totalMarks;
...
export const getExamDetails = async (req, res) => {
  try {
    ...
    // Calculate metadata
    const totalStudents = exam.classroom?.students
      ? exam.classroom.students.filter((s) => s != null).length
      : 0;
