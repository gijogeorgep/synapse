  ...
  try {
    ...
    if (user && (await user.matchPassword(password))) {
      ...
      if (user.role === "admin" || user.role === "superadmin") {
        console.warn(
          `[AUTH] Admin account attempted login via main portal: ${email}`,
        );
        return res.status(403).json({
          message:
            "Admin accounts must login via the admin portal at admin.synapseeduhub.com",
        });
      }
...
export const adminLogin = async (req, res) => {
  ...
  try {
    const user = await User.findOne({ email });
    console.log(
      `Login attempt for: ${email}, found user: ${user ? "yes" : "no"}`,
    );

    if (user) {
      const isMatch = await user.matchPassword(password);
      console.log(
        `[AUTH] Login attempt for ${email} - User found, Password match: ${isMatch}`,
      );

      if (isMatch) {
        if (user.role !== "admin" && user.role !== "superadmin") {
          console.warn(
            `[AUTH] Unauthorized role access attempt: ${email} has role ${user.role}`,
          );
          return res
            .status(403)
            .json({
              message: "Not authorized as administrator. Access denied.",
            });
        }
